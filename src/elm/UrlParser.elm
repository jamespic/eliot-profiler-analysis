{-
Copied from evancz/url-parser - he did say to use it as a starting point
-}
module UrlParser exposing
  ( Parser, string, int, s
  , (</>), map, oneOf, top, custom
  , QueryParser, (<?>), stringParam, intParam, customParam, dateParam, dictParam
  , parsePath, parseHash
  )
...
dateParam : String -> QueryParser (Maybe Date.Date -> b) b
dateParam key =
  customParam key <| Maybe.andThen (Date.fromString >> Result.toMaybe)

dictParam : (String -> Bool) -> QueryParser (Dict String String -> a) a
dictParam filter = QueryParser <| \{ visited, unvisited, params, value } ->
    [ State visited unvisited params (value (Dict.filter (\k -> \v -> filter k) params)) ]
